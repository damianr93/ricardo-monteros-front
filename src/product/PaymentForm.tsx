...
const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const groupItems = (items: Product[]): GroupedItem[] => {
    const grouped = items.reduce<Record<string, GroupedItem>>((acc, item) => {
      ...
      if (acc[key]) {
        ...
        acc[key] = { product: item, quantity: 1 };
      }
      return acc;
    }, {});
  ...
  const groupedItemsForDisplay = groupItems(items);
  ...
  return (
    ...
    >
      <h2 className="text-3xl font-heading text-primary text-center">
        Confirmar Pedido
      </h2>

      <div className="bg-secondary-lightest p-4 rounded-lg shadow-inner space-y-2 border border-secondary-dark">
        <h3 className="text-lg font-semibold text-secondary-darkest">
          Resumen
        </h3>
        <ul className="space-y-1 text-sm text-secondary-darkest">
          {groupedItemsForDisplay.map(({ product, quantity }, i) => (
            <li key={i} className="flex justify-between">
              <span>
                {product.title || product.name}
                <span className="text-secondary-muted"> x{quantity}</span>
                {product.codigo && (
