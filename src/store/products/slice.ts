...
export const productsSlice = createSlice({
  ...
  reducers: {
    ...
    fetchSuccess(state, action: PayloadAction<Product[]>) { state.loading = false; state.list = action.payload },
    fetchFailure(state, action: PayloadAction<string>) { state.loading = false; state.error = action.payload },
    addSuccess(state, action: PayloadAction<Product>) { state.list.push(action.payload) },
